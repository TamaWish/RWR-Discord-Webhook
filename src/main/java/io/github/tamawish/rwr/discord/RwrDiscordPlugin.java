 ...
 *
 * <p>Depends only on the public RWR API surface and Bukkit/Paper at compile time. Both RWR runtime
 * plugin names are soft dependencies; the add-on disables event delivery cleanly when the API
 * service is absent.
 */
public final class RwrDiscordPlugin extends JavaPlugin implements Listener {
  ...
  public void onEnable() {
    ...
    getLogger()
        .info(
            locale.format(
                apiAvailable ? "log.enabled" : "log.enabled-degraded",
                "webhook_state",
                webhookState));
    startupError.ifPresent(
        error -> getLogger().severe("RWR-Discord Webhook configuration is inactive: " + error));
    if (!apiAvailable) {
  ...
  private void registerApiListener() {
    ...
    try {
      ...
      getLogger()
          .warning(
              "RWR API event classes are unavailable; webhook event delivery remains disabled.");
    }
  }
