...
/** Immutable snapshot of RWR-Discord Webhook settings loaded from the add-on data folder. */
public final class DiscordConfig {
  ...
  private final MentionSettings mentions;
  ...
  public static DiscordConfig load(FileConfiguration yaml) {
    ...
    String avatar = blankToEmpty(yaml.getString("webhook.avatar_url", ""));
    ...
    if (yaml.contains("webhook.max-attempts")) {
      maxAttempts =
          clamp(yaml.getInt("webhook.max-attempts"), 1, WebhookService.DEFAULT_MAX_ATTEMPTS);
    } else if (yaml.contains("webhook.max-retries")) {
      maxAttempts =
          clamp(yaml.getInt("webhook.max-retries") + 1, 1, WebhookService.DEFAULT_MAX_ATTEMPTS);
    } else {
    ...
    MentionSettings mentions = MentionSettings.load(yaml);
  ...
  private static void validateWebhookUrl(String value) {
    ...
    try {
      ...
      throw new IllegalArgumentException("webhook.url is not a valid URI", ex);
    }
    String host = uri.getHost();
    boolean discordHost =
        host != null
            && (host.equalsIgnoreCase("discord.com")
                || host.equalsIgnoreCase("discordapp.com")
                || host.toLowerCase(Locale.ROOT).endsWith(".discord.com")
                || host.toLowerCase(Locale.ROOT).endsWith(".discordapp.com"));
    String path = uri.getPath();
    if (!"https".equalsIgnoreCase(uri.getScheme())
        || !discordHost
        || path == null
        || !path.matches("/api/webhooks/\\d+/[^/]+/?")) {
      throw new IllegalArgumentException(
          "webhook.url must be an HTTPS Discord incoming webhook URL");
    }
  }
  ...
  public MentionSettings mentions() {
    return mentions;
  }
}
