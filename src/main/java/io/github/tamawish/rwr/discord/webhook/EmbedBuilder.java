...
public final class EmbedBuilder {
  ...
  public JsonObject buildConfiguration(
      boolean apiAvailable, String pluginVersion, String serverVersion) {
    JsonObject embed = new JsonObject();
    embed.addProperty("title", locale.format("embed.configuration-title"));
    embed.addProperty(
        "description", withLegalLinks(locale.format("embed.configuration-description")));
  ...
  private JsonObject baseEmbed(
    ...
    operationId.ifPresent(id -> fields.add(field(locale.raw("embed.field-operation"), id, true)));
    remaining.ifPresent(
        value -> fields.add(field(locale.raw("embed.field-remaining"), value, true)));
    scheduledResetAt.ifPresent(
        value ->
            fields.add(field(locale.raw("embed.field-scheduled-at"), value.toString(), false)));
    phase.ifPresent(
        p ->
            fields.add(
                field(
                    locale.raw("embed.field-phase"), locale.raw("embed.phase-" + p.name()), true)));
    failure.ifPresent(
        f ->
            fields.add(
                field(
    ...
    message.ifPresent(
        m ->
            fields.add(
                field(locale.raw("embed.field-message"), LocaleService.stripMentions(m), false)));
    embed.add("fields", fields);
