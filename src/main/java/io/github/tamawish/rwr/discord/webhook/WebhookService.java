...
public final class WebhookService {
  ...
  private static final Pattern WEBHOOK_SECRET =
      Pattern.compile(
  ...
  private final ConcurrentLinkedQueue<PendingNotification> admissions =
  ...
  public WebhookService(
    ...
    this.queue =
        new PersistentNotificationQueue(
          ...
          .ifPresent(
              detail -> {
                ...
                throw new IllegalStateException(
                    "Durable webhook queue is unavailable: " + safeDetail);
              });
    }
  }
  ...
  public void enqueue(NotificationCategory category, String operationId, JsonObject embed) {
    ...
    if (queue.size() + admitted > config.queueCapacity()) {
      ...
      plugin
          ...
          .warning(
              locale.format(
                  "log.queue-full", "category", category.name(), "operation", operationId));
      return;
    }
  ...
  static long retryDelayMs(DeliveryResult result, int attemptNumber) {
    ...
    long ordinaryDelay =
        result.retryAfterMs() > 0L ? result.retryAfterMs() : computeBackoffMs(attemptNumber);
