...
class ResetEventListenerTest {
  ...
  void forwardsEveryEnabledScheduledWarningWithoutAnOperationId() {
    ...
    when(embeds.buildWarning("resource", "resource_world", 10, scheduledAt)).thenReturn(embed);
  ...
  void warningToggleAndDegradedModeSuppressDelivery() {
    ...
    when(plugin.discordConfig()).thenReturn(config);
    when(plugin.webhooks()).thenReturn(webhooks);
    ResourceWorldResetWarningEvent event =
        new ResourceWorldResetWarningEvent("resource", "resource_world", 5, Instant.EPOCH);
  ...
  void cancellationUsesItsOwnConfiguredCategory() {
    ...
    new ResetEventListener(plugin)
        .onPostReset(
            new ResourceWorldPostResetEvent(
                "operation",
                "resource",
                "resource_world",
                ResetPhase.FAILED,
