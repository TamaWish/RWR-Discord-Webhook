...
class EmbedBuilderTest {
  ...
  void warningContainsScheduleFieldsAndNoOperationField() {
    ...
    assertThat(fieldNames(fields))
        .contains(
            "embed.field-world-id",
            "embed.field-world-name",
            "embed.field-remaining",
            "embed.field-scheduled-at")
  ...
  void configurationNoticeReportsApiAddonAndServer() {
    ...
    JsonArray fields = embed.getAsJsonArray("fields");
    assertThat(fieldNames(fields))
        .contains("embed.field-api-status", "embed.field-addon-version", "embed.field-server");
  ...
  void terminalRetainsOperationAndDiagnosticFields() {
    ...
    when(locale.format(
            "embed.failure-description", "world_name", "resource_world", "world_id", "resource"))
        .thenReturn("Failed");

    JsonObject embed =
        embeds.buildTerminal(
